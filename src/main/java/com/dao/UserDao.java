...
public class UserDao {
	...
	public UserBean findByEmail(String email) {
		try {
			return stmt.queryForObject("SELECT * FROM users WHERE email=?", new BeanPropertyRowMapper<>(UserBean.class),
					new Object[] { email });
		} catch (Exception e) {
			return null;
		}
	...
	public boolean verifyOtp(String email, String otp) {
		Integer count = stmt.queryForObject("SELECT count(*) FROM users WHERE email=? AND otp=?", Integer.class,
				new Object[] { email, otp });
